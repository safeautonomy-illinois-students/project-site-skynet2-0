---
pagetitle: "Autonomous Drone Racing"
format:
  html:
    toc: false
    page-layout: full
...
# Autonomous Drone Racing {.title}

::: {.subtitle}
Let the drone flow like the wind
:::
...
![](static/images/CustomImage/block7.png){fig-align="center" width="800"}
...
For perception, we utilize the motion capture to find the position waypoints 
...
gates in minimum lap time. This will be done by analyzing where the center of the gates 
are in the world and creating a smooth path that goes through all of them while not 
hitting anything else. An algorithm that finds the most efficient curvature routes
combined with the most efficient velocity/acceleration will generate the list of waypoints.
The list of waypoints will be given to the control unit.
...
Another way we could implement this is through the use of Reinforcement Learning. We could
train a model on different sets of tracks and after training, allow it to make
the decision on the waypoints by itself. If successful, this would replace either the path
planning or the local controller.
